...
# runs the pdf scenario (bob receiving, alice sending) with encrypted
# messaging, then prints all three outputs so they can be compared against
# the templates in the pdf.
#
...
SERVER_PID=$!
sleep 1.5   # keypair generation takes a moment

# bob: connects, logs in, sits long enough to receive alice's messages, quits
( echo "connect 127.0.0.1 $PORT"; sleep 0.8
  echo "login bob";               sleep 5
...
( echo "connect 127.0.0.1 $PORT";   sleep 0.8
  echo "login alice";               sleep 0.6
  echo "who";                       sleep 0.6
  echo "broadcast Hello all!";      sleep 0.6
  echo "private bob Lets talk";     sleep 0.6
